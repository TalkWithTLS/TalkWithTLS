...
@pytest.mark.parametrize("app1, app2, app1_args, app2_args, app1_res, app2_res", [
    ...
    ('test_openssl', 'test_openssl', ' -s -k 1 --msgcb ', ' -k 1 --msgcb ', 0, 0),

    # Test with kexchange set as string using SSL_set1_groups_list API on client
    ('test_openssl', 'test_openssl', ' -s -k 1 --msgcb ', ' -k 3 --msgcb ', 0, 0),

    #('test_openssl', 'test_openssl', ' -s -k 2 --msgcb ', ' -k 2 m ', 0, 0), #Test with all FFDHE alg

    # Test with kexchange set as string using SSL_set1_groups_list API on both client and server
    ('test_openssl', 'test_openssl', ' -s -k 3 --msgcb ', ' -k 3 m ', 0, 0), #ECDHE kexch
    #('test_openssl', 'test_openssl', ' -s -k 4 --msgcb ', ' -k 4 m ', 0, 0), #FFDHE kexch

    # TODO Pass SSL error code to valide in testcode
    # Test with only FFDHE kexch alg on TLS1.2 server and client
    #('test_openssl', 'test_openssl', ' -s -k 2 -v 12 --msgcb ', ' -k 2 -v 12 --msgcb ', 255, 255),
    # Test with only FFDHE kexch alg on TLS1.2 client
    #('test_openssl', 'test_openssl', ' -s -k 2 -v 13 --msgcb ', ' -k 2 -v 12 --msgcb ', 255, 255),
])
