...
BIO *get_custom_bio(int fd)
{
    BIO_METHOD* bmethod = NULL;
    const BIO_METHOD* bmethod_base;
    BIO *bio;

    bmethod = BIO_meth_new(BIO_TYPE_DGRAM, "socket");
    if (!bmethod) {
        printf("BIO meth new failed\n");
        return NULL;
    }

    bmethod_base = BIO_s_datagram();
    if (!bmethod_base) {
        printf("BIO meth s datagram failed\n");
        goto err;
    }

    BIO_meth_set_read(bmethod, BIO_meth_get_read(bmethod_base));
    BIO_meth_set_write(bmethod, BIO_meth_get_write(bmethod_base));
    BIO_meth_set_ctrl(bmethod, BIO_meth_get_ctrl(bmethod_base));
    BIO_meth_set_create(bmethod, BIO_meth_get_create(bmethod_base));
    BIO_meth_set_puts(bmethod, BIO_meth_get_puts(bmethod_base));
    BIO_meth_set_destroy(bmethod, BIO_meth_get_destroy(bmethod_base));
...
int update_dtls_client_bio(SSL *ssl, const char *serv_ip, uint16_t serv_port)
{
    ...
    if (BIO_ctrl(SSL_get_wbio(ssl), BIO_CTRL_DGRAM_SET_MTU, 1500, NULL) != 1500) {
